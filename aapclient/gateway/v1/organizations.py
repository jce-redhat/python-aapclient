...
def _format_organization_data(
    organization_data: Dict[str, Any], use_utc: bool = False, output_format: str = "table", client_manager=None
) -> Dict[str, Any]:
    ...
    if client_manager and org_id:
        try:
            ...
            if controller_org:
                ...
                gc_response = controller_client.get(
                    f"/api/controller/v2/organizations/{controller_org_id}/galaxy_credentials/"
                )
                ...
                # Get instance groups
                ig_response = controller_client.get(
                    f"/api/controller/v2/organizations/{controller_org_id}/instance_groups/"
                )
...
def show_organization(
    console: Console, output_format: str, utc: bool, organization_name: Optional[str], id: Optional[int]
) -> None:
...
def create_organization(
    ...
    galaxy_credential: tuple,
    instance_group: tuple,
) -> None:
    ...
    if execution_environment or galaxy_credential or instance_group:
        ...
        try:
            ...
            for gc_name in galaxy_credential:
                ...
                controller_client.post(
                    f"/api/controller/v2/organizations/{controller_org_id}/galaxy_credentials/", json={"id": gc_id}
                )
            ...
            for ig_name in instance_group:
                ...
                controller_client.post(
                    f"/api/controller/v2/organizations/{controller_org_id}/instance_groups/", json={"id": ig_id}
                )
...
@click.option("--max-hosts", type=int, help="Maximum number of hosts for this organization")
@click.option("--execution-environment", help="Default execution environment name or ID")
@optgroup.group("Galaxy Credentials", cls=MutuallyExclusiveOptionGroup, help="Add or remove galaxy credentials")
@optgroup.option(
    "--add-galaxy-credential", multiple=True, help="Add galaxy credential name or ID (can be used multiple times)"
)
@optgroup.option(
    "--remove-galaxy-credential", multiple=True, help="Remove galaxy credential name or ID (can be used multiple times)"
)
@optgroup.group("Instance Groups", cls=MutuallyExclusiveOptionGroup, help="Add or remove instance groups")
@optgroup.option(
    "--add-instance-group", multiple=True, help="Add instance group name or ID (can be used multiple times)"
)
@optgroup.option(
    "--remove-instance-group", multiple=True, help="Remove instance group name or ID (can be used multiple times)"
)
@update_command
def set_organization(
    ...
    max_hosts: Optional[int],
    execution_environment: Optional[str],
    add_galaxy_credential: tuple,
    remove_galaxy_credential: tuple,
    add_instance_group: tuple,
    remove_instance_group: tuple,
) -> None:
    ...
    has_controller_updates = bool(
        execution_environment
        or add_galaxy_credential
        or remove_galaxy_credential
        or add_instance_group
        or remove_instance_group
    )
    ...
    formatted_data = _format_organization_data(
        updated_organization, use_utc=False, output_format="table", client_manager=client_manager
    )
    show_details_table(console, formatted_data)
