...
@list_command(
    default_limit=20, sort_fields=["id", "name", "organization", "image", "created", "modified"], default_sort="id"
)
    ...
    try:
        ...
        # Format the data for table output
            ...
            for ee in execution_environments:
                ...
                rows.append([str(ee["id"]), ee.get("name", ""), ee.get("image", ""), organization_name])
            ...
            if not show_all and data.get("count", 0) > len(execution_environments):
                console.print(
                    f"[dim]Showing {len(execution_environments)} of {data['count']} total execution environments[/dim]"
                )
...
@click.argument(
    ...
    required=False,
    callback=validate_resource_identifier,
)
...
def set_execution_environment(
    console, execution_environment_name, id, set_name, image, description, organization, credential, pull_policy
):
...
def _format_execution_environment_data(
    ee: Dict[str, Any], use_utc: bool = False, output_format: str = "table"
) -> Dict[str, Any]:
