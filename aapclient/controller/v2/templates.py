...
def list_templates(
    ...
    if show_all:
        ...
        data = {"count": len(templates), "results": templates}
    else:
        # Regular pagination
    ...
    for template in templates:
        ...
        last_run = (
            format_datetime_rich(last_job, utc, output_format)
            if last_job
            else ("Never" if output_format in ["json", "yaml"] else "Never")
        )
...
@show_command
def show_template(
    console: Console, output_format: str, utc: bool, template_name: Optional[str], id: Optional[int]
) -> None:
...
@click.option(
    "--webhook-service",
    type=click.Choice(["gitlab", "github", "bitbucket_dc"]),
    help="Webhook service (required if --enable-webhook)",
)
...
def _build_template_data(client, name: str, **kwargs) -> Dict[str, Any]:
    ...
    data = {"name": name, "job_type": kwargs["job_type"], "playbook": kwargs["playbook"]}
        ...
        for cred_name in kwargs["credential"]:
            try:
                ...
                assoc_response = client.post(
                    f"{CONTROLLER_API_VERSION_ENDPOINT}job_templates/{template_id}/credentials/", json={"id": cred_id}
                )
    ...
    if kwargs.get("instance_group"):
        for ig_name in kwargs["instance_group"]:
            try:
                ...
                assoc_response = client.post(
                    f"{CONTROLLER_API_VERSION_ENDPOINT}job_templates/{template_id}/instance_groups/", json={"id": ig_id}
                )
                if assoc_response.status_code not in [HTTP_CREATED, HTTP_NO_CONTENT]:
...
def _format_template_data(
    template: Dict[str, Any], use_utc: bool = False, client=None, output_format: str = "table"
) -> Dict[str, Any]:
...
@optgroup.group("Privileged Escalation", cls=MutuallyExclusiveOptionGroup, help="Control privileged escalation setting")
...
@optgroup.group("Concurrent Jobs", cls=MutuallyExclusiveOptionGroup, help="Control concurrent jobs setting")
...
@optgroup.group("Fact Storage", cls=MutuallyExclusiveOptionGroup, help="Control fact storage setting")
...
@optgroup.group("Show Changes", cls=MutuallyExclusiveOptionGroup, help="Control diff mode changes display")
...
@optgroup.group(
    "Instance Group Fallback", cls=MutuallyExclusiveOptionGroup, help="Control instance group fallback behavior"
)
