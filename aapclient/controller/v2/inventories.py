...
from aapclient.common.functions import resolve_inventory_name, resolve_organization_name, format_variables_display
...
@list_command(
    default_limit=20, sort_fields=["id", "name", "organization", "type", "created", "modified"], default_sort="id"
)
def list_inventories(console, output_format, utc, limit, offset, organization, show_all, sort_by, reverse):
    ...
    if show_all:
        ...
        data = {"count": len(inventories), "results": inventories}
    else:
        # Regular pagination
...
def create_inventory(
    console, name, organization, description, variables, prevent_instance_group_fallback, instance_groups
):
...
def delete_inventory(console, inventory_name, id):
    ...
    client.delete(f"{CONTROLLER_API_VERSION_ENDPOINT}inventories/{inventory_id}/")
...
def show_inventory_variables(console, output_format, utc, inventory_name, id):
    ...
    if output_format == "json":
        ...
        data = {"Inventory": inventory_data["name"], "Variables": variables_yaml}
        show_details_table(console, data)
        ...
        try:
            ...
            if assoc_response.status_code not in [HTTP_OK, HTTP_CREATED, 204]:
                association_errors.append(
                    f"Failed to associate instance group '{ig_name}': HTTP {assoc_response.status_code}"
                )
    ...
    for ig_name in instance_groups:
        try:
            ...
            if disassoc_response.status_code not in [HTTP_OK, HTTP_CREATED, 204]:
                association_errors.append(
                    f"Failed to remove instance group '{ig_name}': HTTP {disassoc_response.status_code}"
                )
...
def _format_inventory_data(
    inventory: Dict[str, Any], use_utc: bool = False, client=None, output_format: str = "table"
) -> Dict[str, Any]:
        ...
        if inventory_id:
            try:
                ig_response = client.get(
                    f"{CONTROLLER_API_VERSION_ENDPOINT}inventories/{inventory_id}/instance_groups/"
                )
