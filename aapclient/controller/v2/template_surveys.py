...
from aapclient.common.constants import CONTROLLER_API_VERSION_ENDPOINT, HTTP_OK, HTTP_NO_CONTENT
    ...
    try:
        ...
        if response.status_code == HTTP_OK:
            ...
            if existing_survey.get("spec"):
                show_error_message(
                    console,
                    "Survey already exists for this job template. Use 'survey question add' to add more questions.",
                )
                import sys
        ...
        if response.status_code == HTTP_OK:
            ...
            if enabled:
                ...
                if template_response.status_code != HTTP_OK:
                    show_error_message(
                        console, f"Survey created but failed to enable it: HTTP {template_response.status_code}"
                    )
                    import sys

                    sys.exit(1)
                show_success_message(
                    console, f"Survey created with 1 question and enabled for job template {job_template_id}"
                )
            else:
...
@optgroup.group("Survey State", cls=MutuallyExclusiveOptionGroup, help="Control survey enabled/disabled state")
...
def set_survey(console, template_name, id, name, description, enabled, disabled):
    ...
    try:
        ...
        if enabled or disabled:
            ...
            if template_response.status_code != HTTP_OK:
                show_error_message(
                    console, f"Failed to update survey enabled status: HTTP {template_response.status_code}"
                )
                import sys
...
def add_question(
    ...
    try:
        ...
        if response.status_code == HTTP_OK:
            ...
            current_survey = {"name": "", "description": "", "spec": []}
...
def delete_question(console, template_name, question_index, id):
    ...
    try:
        ...
        survey_spec.pop(question_index - 1)
        current_survey["spec"] = survey_spec
        ...
        if not survey_spec:
            ...
            if response.status_code == HTTP_OK:
                show_success_message(
                    console, f"Question {question_index} removed. Survey now has {len(survey_spec)} question(s)"
                )
            else:
