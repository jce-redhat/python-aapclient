...
from aapclient.common.functions import resolve_instance_name, resolve_instance_group_name, resolve_credential_name
...
@click.option(
    "--peers-from-control-node",
    is_flag=True,
    help="Enable peers from control nodes (sets peers_from_control_nodes=true)",
)
@create_command
def create_instance(
    console, hostname, listener_port, instance_type, enabled, managed_by_policy, peers_from_control_node
):
...
@optgroup.group("Instance State", cls=MutuallyExclusiveOptionGroup, help="Control instance enabled/disabled state")
...
@optgroup.group(
    "Peers from Control Nodes", cls=MutuallyExclusiveOptionGroup, help="Control peers from control nodes setting"
)
...
@optgroup.group("Management by Policy", cls=MutuallyExclusiveOptionGroup, help="Control management by policy setting")
...
def set_instance(
    ...
    enable,
    disable,
    enable_peers_from_control_nodes,
    disable_peers_from_control_nodes,
    enable_manage_by_policy,
    disable_manage_by_policy,
):
...
@click.argument(
    "instance_group_name", metavar="<instance_group>", required=False, callback=validate_resource_identifier
)
...
@click.option(
    "--policy-instance-percentage", type=int, help="Percentage of instances to maintain (instance groups only)"
)
...
def create_instance_group(
    ...
    instance_group_data = {"name": name, "is_container_group": is_container_group}
...
def _format_instance_data(
    instance: Dict[str, Any], use_utc: bool = False, output_format: str = "table"
) -> Dict[str, Any]:
    ...
    data["Managed by Policy"] = format_value_for_output(
        instance.get("managed_by_policy", False), "managed_by_policy", output_format
    )
...
def _format_instance_group_data(
    instance_group: Dict[str, Any], use_utc: bool = False, output_format: str = "table"
) -> Dict[str, Any]:
