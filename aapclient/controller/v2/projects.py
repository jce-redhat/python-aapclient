...
def list_projects(console, output_format, utc, limit, offset, organization, show_all, sort_by, reverse):
    ...
    try:
        ...
        params = {"page_size": limit, "page": (offset // limit) + 1 if offset else 1}
...
def _format_project_data(
    project: Dict[str, Any], use_utc: bool = False, client=None, output_format: str = "table"
) -> Dict[str, Any]:
...
@click.option("--scm-type", type=click.Choice(["git", "svn", "insights", "archive"]), help="Update source control type")
...
@optgroup.group("SCM Track Submodules", cls=MutuallyExclusiveOptionGroup, help="Control SCM submodule tracking")
...
@optgroup.group("SCM Update on Launch", cls=MutuallyExclusiveOptionGroup, help="Control SCM update on job launch")
...
@optgroup.group("SCM Branch Override", cls=MutuallyExclusiveOptionGroup, help="Control SCM branch override capability")
...
@optgroup.group("SCM Clean", cls=MutuallyExclusiveOptionGroup, help="Control SCM clean on update")
...
@optgroup.group("SCM Delete on Update", cls=MutuallyExclusiveOptionGroup, help="Control SCM delete on update")
...
def set_project(
    ...
    scm_update_cache_timeout,
    timeout,
    enable_scm_track_submodules,
    disable_scm_track_submodules,
    enable_scm_update_on_launch,
    disable_scm_update_on_launch,
    enable_scm_allow_branch_override,
    disable_scm_allow_branch_override,
    enable_scm_clean,
    disable_scm_clean,
    enable_scm_delete_on_update,
    disable_scm_delete_on_update,
):
    ...
    try:
        ...
        if signature_validation_credential:
            project_data["signature_validation_credential"] = resolve_credential_name(
                client, signature_validation_credential
            )
...
def sync_project(console, project_name, id, wait, timeout):
    ...
    try:
        ...
        if response.status_code in [200, 202]:
            ...
            if update_id:
                show_success_message(
                    console, f"Project sync started for '{project_name_actual}' (Update ID: {update_id})"
                )
